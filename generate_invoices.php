<?php
/* THIS SCRIPT GENERATE INVOICES FOR GIVEN CONTRACT UP TO THE current MONTH */
/* IT'S A SECOND VERSION, FIRST WAS gen_invoices.php */
include ("login_re.php"); 

		$id= $_REQUEST['id'];		// Contract ID
		$today_m = (int)date("m"); // only current month is of intertest
		
		$today_y = (int)date("y"); // only current year is of intertest
			...
			// 1. Get contract data	
				$contract_sql='SELECT contract.id,VAT,contract.currency,conditions.pay_terms,conditions.min_payment 
								...
								WHERE CURDATE()<exp_date AND contract.isValid=1 AND contract.id='.$id;
			...
			// 2. LIST INVOICES IN THE PERIOD FOR THIS CONTRACT	
					$VAT=$row[0];
					$cur=$row[1]; //NOW it points to the contract. Although
					$pay_terms=$row[2];
					$min=$row[3];
